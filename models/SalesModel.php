...
{
    /** Get sales for a specific date and outlet */
    public function getByDateAndOutlet(string $date, int $outlet_id): array
    {
        return $this->db->read(
            "SELECT s.*, p.Name AS Product_Name, p.Unit, c.Name AS Category_Name,
             ...
             WHERE s.Outlet_ID = ? AND s.Sales_date = ?
             ORDER BY c.Name, p.Name",
            [$outlet_id, $date]
        );
    }

    /** Get daily totals for a date and outlet */
    ...
    /** Get recent sales dates for an outlet */
    ...
    public function create(int $outlet_id, int $user_id, int $product_id, string $date, int $quantity_sold, float $unit_price): int
    {
        return $this->db->insert(
            "INSERT INTO Sales (Outlet_ID, User_ID, Product_ID, Sales_date, Quantity_sold, Unit_Price)
             VALUES (?, ?, ?, ?, ?, ?)",
            [$outlet_id, $user_id, $product_id, $date, $quantity_sold, $unit_price]
        );
    }

    /** Lock all sales for a specific date and outlet */
    public function lockByDate(int $outlet_id, string $date): int
    {
        return $this->db->write(
            "UPDATE Sales SET Locked_status = 1
             WHERE Outlet_ID = ? AND Sales_date = ? AND Locked_status = 0",
            [$outlet_id, $date]
        );
    }
